const DIAG_STORAGE_KEY = 'padre_diagnostics';
...
class Diagnostics {
  ...
  learnPopupPattern(popupNode) {
    ...
    if (this.domPatterns.size > DIAG_MAX_PATTERNS) {
      ...
      const sorted = [...this.domPatterns.entries()]
        .sort((a, b) => a[1].count - b[1].count);
      for (let i = 0; i < sorted.length - DIAG_MAX_PATTERNS; i++) {
        this.domPatterns.delete(sorted[i][0]);
      }
    }
  ...
  recordDetectionMiss(node) {
    ...
    if (this.domSnapshots.length > DIAG_MAX_SNAPSHOTS) {
